			...
			for (int i = 1; i <= howMany; i++) {
				...
				section("Condition #$i") {
					...
					if(myCapab) {
						...
						if(!(xCapab in ["Time of day", "Certain Time", "Days of week", "Mode", "Rule state"])) {
							def thisDev = "rDev$i"
							getDevs(xCapab, thisDev)
							def myDev = settings.find {it.key == thisDev}
							if(myDev) if(myDev.value.size() > 1) getAnyAll(thisDev)
							if(xCapab in ["Temperature", "Humidity", "Illuminance", "Dimmer level"]) getRelational(thisDev)
						}
						getState(xCapab, i)
					}
				}
...
def getCapab(myCapab) {  
	def myOptions = ["Switch", "Motion", "Acceleration", "Contact", "Presence", "Lock", "Temperature", "Humidity", "Illuminance", "Time of day", "Certain Time", "Days of week", "Mode", "Dimmer level"]
...
private conditionLabelN(i) {
	def result = ""
        ...
        if(thisCapab.value == "Time of day") result = "Time between " + timeIntervalLabel()
			...
			if(thisCapab.value in ["Temperature", "Humidity", "Illuminance", "Dimmer level"]) result = result + " " + thisRel.value + " "
